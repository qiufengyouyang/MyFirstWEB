...
public class DatabaseHelper {
	...
	/**
	 * 开启事务:
	 * 有可能从 ThreadLocal 中根本拿不到 Connection，
	 * 因为此时的 Connection 是从 DataSource 中获取的（这是非事务的情况），
	 * 只要执行了 beginTransaction() 方法，
	 * 就会从 DataSource 中获取一个 Connection，
	 * 然后将事务自动提交功能关闭，最后往 ThreadLocal 中放入一个 Connection。
	 */
